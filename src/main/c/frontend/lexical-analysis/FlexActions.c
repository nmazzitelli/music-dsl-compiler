...
CompilationStatus NoteClassLexemeAction() {
	Token * token = createToken(_lexicalAnalyzer, NOTE_CLASS);
	token->semanticValue->string = strdup(token->lexeme);
	_logTokenAction(__FUNCTION__, token);
	CompilationStatus status = pushToken(_lexicalAnalyzer, token);
	destroyToken(token);
...
CompilationStatus PitchLexemeAction() {
	Token * token = createToken(_lexicalAnalyzer, PITCH);
	token->semanticValue->string = strdup(token->lexeme);
	_logTokenAction(__FUNCTION__, token);
	CompilationStatus status = pushToken(_lexicalAnalyzer, token);
	destroyToken(token);
...
CompilationStatus StringLexemeAction() {
	...
	strncpy(value, token->lexeme + 1, token->length - 2);
